...
- url 형식: file:/// + 절대경로 (역슬래시 아닌 정방향 슬래시 / 사용)
...
- data.json의 frames 배열에 모든 파일을 등록
  - forceIframe: false / padding: 0 / customCss: "body { overflow-x: hidden; }"
...
[자동 확인 사항]
1. Obsidian Vault 위치 탐지 (Documents/Obsidian Vault 등 일반적인 경로 확인)
...
- `forceIframe`이 `false`인지 확인 — `true`면 보안 제한된 iframe을 사용해서 로컬 JS가 안 됨
- URL 경로에 역슬래시(`\`) 대신 정방향 슬래시(`/`)를 사용했는지 확인
- HTML 파일이 해당 경로에 실제로 존재하는지 확인
- 인터넷 연결 확인 — CDN에서 React/Babel을 다운로드해야 함
...
<details>
<summary><b>Reading View에서만 동작하나요?</b></summary>
<br/>
